...
for person, features_person in my_dataset.iteritems():
    ...
    if features_person['exercised_stock_options'] == 'NaN' \
            or features_person['total_stock_value'] == 'NaN':
        features_person['ratio_exercised_total_stock'] = 'NaN'
    else:
        features_person['ratio_exercised_total_stock'] = float(features_person['exercised_stock_options'])\
                                                         / float(features_person['total_stock_value'])
...
def tune(selector, classifier, X=features, y=labels, print_workflow=False, print_best=False):
    '''
    Tune the parameters of SelectKBest and a given prediction estimator using Pipeline and GridSearchCV.

    :param selector: e.g SelectKBest
    :param classifier: e.g GaussianNB(), DecisionTreeClassifier()
    :param X: features set
    :param y: labels set
    :param print_workflow: print verbose and computing time
    :param print_best: print best parameters found
...
#tune(SelectKBest(), GaussianNB(), print_best=True)
...
k_best = tune(SelectKBest(), GaussianNB())['select_features__k']

#create and fit classifier
pln=try_classifier(SelectKBest(k=k_best), GaussianNB())
